...
<AccordionGroup>
  ...
  <Accordion title="VoyageAI">
    ...
    **Config Options:**
    ...
    - `model` (str): Model to use. Default: `voyage-2`. Current options include `voyage-4`, `voyage-4-lite`, `voyage-4-large`, `voyage-code-4`, `voyage-3.5`, `voyage-3.5-lite`, the contextualized `voyage-context-4`, and the multimodal `voyage-multimodal-3`. See the [VoyageAI model overview](https://docs.voyageai.com/docs/embeddings) for the full catalog.
    - `input_type` (str): Type of input. Options: `document` (for storage), `query` (for search)
    - Contextualized models (`voyage-context-*`) embed each input string as its own independent document — one vector per input. Cross-input contextualization is not used, so `embed_many`-style batches of unrelated texts behave the same as with plain embedding models.
