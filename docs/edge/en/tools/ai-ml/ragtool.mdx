...
<AccordionGroup>
  ...
  <Accordion title="VoyageAI by MongoDB">
    ...
    **Config Options:**
    - `api_key` (str): VoyageAI by MongoDB API key
    - `model` (str): Model to use. Default: `voyage-2`. Current options: `voyage-4`, `voyage-4-lite`, `voyage-4-large`, `voyage-code-4`, `voyage-4-nano`, `voyage-3.5`, `voyage-3.5-lite`, `voyage-context-4` (contextualized), `voyage-multimodal-3`, `voyage-finance-2`, `voyage-law-2`. Legacy names (e.g. `voyage-3`, `voyage-code-3`, `voyage-large-2`) still work.
    ...
    <Note>
    Contextualized models (`voyage-context-4`, `voyage-context-3`) are routed to
    the `contextualized_embed` endpoint automatically. Inputs are sent as a flat
    `list[str]` with `enable_auto_chunking=True` and `chunk_size=32000`, so each
    input is embedded independently as a single vector. Query embeddings
    (`input_type="query"`) disable auto-chunking, as required by the API.
    </Note>
