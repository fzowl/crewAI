...
<AccordionGroup>
  ...
  <Accordion title="VoyageAI by MongoDB">
    ...
    **خيارات الإعداد:**
    - `api_key` (str): مفتاح API لـ VoyageAI by MongoDB
    - `model` (str): النموذج المستخدم. القيمة الافتراضية: `voyage-2`. الخيارات الحالية: `voyage-4`، `voyage-4-lite`، `voyage-4-large`، `voyage-code-4`، `voyage-4-nano`، `voyage-3.5`، `voyage-3.5-lite`، `voyage-context-4` (سياقي)، `voyage-multimodal-3`، `voyage-finance-2`، `voyage-law-2`. الأسماء القديمة (مثل `voyage-3`، `voyage-code-3`، `voyage-large-2`) لا تزال تعمل.
